...
from paper_tran.arxiv_crawler.async_translator import async_translate
from paper_tran.arxiv_crawler.categories import parse_categories
...
class Paper:
    ...
    pdf_link: str | None = None  # 新增 PDF 链接属性
    ...
    def from_row(cls, row: sqlite3.Row):
        return cls(
            ...
            pdf_link=row["pdf_link"],  # 新增 PDF 链接读取
        )
    @property
    ...
    @property
    def pdf_url(self):
        return self.pdf_link  # 返回存储的 PDF 链接
...
class PaperDatabase:
    def __init__(self, db_path="dbname=paper_db user=postgres password=difyai123456 host=localhost"):
        self.conn = psycopg2.connect(db_path)
        self.conn.autocommit = True  # PostgreSQL 中需要设置 autocommit
        self._create_table()
        ...
        with self.conn.cursor() as cursor:
            data_to_insert = [
                (
                    ...
                    paper.comments,
                    paper.pdf_link,  # 新增 PDF 链接存储
                    datetime.now()  # PostgreSQL 会自动处理时区
                )
                for paper in papers
            ]
            cursor.executemany(
                ...
                (authors, abstract, title, categories, first_submitted_date, first_announced_date, title_translated, abstract_translated, comments, pdf_link,update_time)
                ...
                ON CONFLICT (id) DO UPDATE SET
                    ...
                    first_announced_date = EXCLUDED.first_announced_date,
    ...
    def count_new_papers(self, papers: Iterable[Paper]) -> int:
        ...
        with self.conn.cursor() as cursor:
            for paper in papers:
                cursor.execute(
                    "SELECT * FROM papers WHERE url = %s",
                    (paper.url,),
                )
                if cursor.fetchone():
                    break
    ...
    def fetch_papers_on_date(self, date: datetime) -> list[Paper]:
        with self.conn.cursor() as cursor:
            cursor.execute(
                "SELECT * FROM papers WHERE first_announced_date = %s",
                (date.strftime("%Y-%m-%d"),),
            )
    ...
    def newest_update_time(self) -> datetime:
        ...
        with self.conn.cursor() as cursor:
            cursor.execute(
                """
                SELECT MAX(update_time) as max_updated_time FROM papers
                WHERE first_announced_date = (SELECT MAX(first_announced_date) FROM papers)
                """
            )
            time = cursor.fetchone()["max_updated_time"]
            return time
    ...
    async def translate_missing(self, langto="zh-CN"):
        ...
        async def worker(url, title, abstract):
            ...
            with self.conn.cursor() as cursor:
                cursor.execute(
                    "UPDATE papers SET title_translated = %s, abstract_translated = %s WHERE url = %s",
...
class PaperExporter:
    def __init__(
        ...
        database_path="dbname=paper user=postgres password=difyai123456 host=localhost",
    ):
